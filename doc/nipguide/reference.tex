...
If you press \ct{i} with the keyboard focus on the image you will
zoom in on the pixel your mouse pointer is over. Press \ct{o} to zoom
out again, or press the number keys \ct{1}, \ct{2}, \ct{4} and \ct{8}
to jump straight to a particular magnification. If you hold down the
Ctrl key while pressing these numbers, \nip{} will zoom out by
that amount. If you press \ct{0} (the number zero), then \nip{} will pick
...
processing operation you want from the \ct{Toolkits} menu.
...
changes to a workspace after loading it.
Suppose the workspace \ct{test.ws} has a row called \ct{A1} with the value 12.
Then entering:
...
recalculation, \nip{} will change the value of \ct{A1} to be 45. You can use
\ct{--set} to create new symbols as well.
...
\begin{verbatim}
nip2 -bp --set 'A1=Image_file "fred.jpg"' --set main=A10 -o fred.jpg process.ws
\end{verbatim}

\noindent
This will start \nip{} in batch (ie. no GUI) mode (the \ct{-b} switch), 
load \ct{myworkspace.ws}, change \ct{A1} to load another file, set \ct{main}
to be the value of A10

and save the value of \ct{A10} to \ct{fred.jpg} (the
\ct{-p} switch).
...
\noindent
This will expand to either the value of the \ct{\$FILE} environment variable,
or the empty list if \ct{\$FILE} is not defined. Now, if that was row
\ct{B27}, change the row that loads the workspace's input image to be:
...
\noindent
And you now have a workspace that processes the image \ct{default.jpg} if you
...
\noindent
And it will process \ct{another.jpg} instead. You can use the
\ctr{Widgets}\ct{File Chooser} widget to make it a little prettier.
