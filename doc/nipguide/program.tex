...
the directories listed in \ct{Start path}. You can change \ct{Start path}
in the \ct{Preferences} workspace. By default, the start path lists
...
functions as parameters, store functions in lists, and so on. You can compose
functions with the \ct{@} operator. For example, for two functions of one
argument \ct{f} and \ct{g}:
...
\begin{verbatim}
/* foldr fn st l: fold up list l, right to
 * left with function fn and start value st
 */
