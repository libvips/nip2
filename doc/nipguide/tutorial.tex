...
added to). Click on the \ct{25 GB free} text and it toggles between showing
...
\ct{Pin up} box to have the dialog remain after you press OK. Click on
\ct{Show thumbnails} and \nip{} will try to display thumbnail images for
...
There are three rows visible here, \ct{A1}, \ct{A2} and \ct{A3}. Each row
...
to the left of the slider and press Return). See \fref{fg:rotate}.
...
\aref{sec:operators} for a list of the differences. Try
multiplying the joined images by a small amount (eg. type something like
\ct{A5 * 1.2} and press Return).
...
back by restarting \nip{} and clicking on \ctr{File}\ct{Recover After Crash}.
There are a lot of preferences (perhaps too many), see \aref{sec:config}.
