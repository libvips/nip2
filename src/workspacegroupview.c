	...
	GtkWidget *page, guint page_num, gpointer user_data )
{
	...
	printf( "mainw_page_added_cb:\n" );
	icontainer_child_add( ICONTAINER( mainw->wsg ), ICONTAINER( ws ), -1 );
}
	...
	GtkWidget *page, int x, int y, gpointer user_data )
{
	...
	if( !(wsg = workspacegroup_new( wsr, "untitled" )) )
		return( NULL );
	...
	Mainwtab *old_tab, Workspace *ws, gboolean trim )
{
	...
	/* If the mainw has a single, empty, unmodified workspace in
